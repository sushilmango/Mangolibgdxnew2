...
public class BookRead implements Screen, InputProcessor {
	...
	public void update (float deltaTime) {
		if (Gdx.input.justTouched()) {
			...
			if (OverlapTester.readforme(read_to_me,touchPoint.x,touchPoint.y)) {
				...
				bundle.putString("Activity", ScreenEnum.SCREEN_2.getValue());
	...
	public void draw (float deltaTime) {
		...
		batcher.draw(Assets.detailimage, detailimage.x, detailimage.y, detailimage.getWidth(), detailimage.getHeight());
		batcher.end();
	}
